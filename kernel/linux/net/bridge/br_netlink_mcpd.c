...
//<< [CTFN-3G-001-4] evan : Support 3G feature without xDSL driver (the function br_igmp_mc_fdb_add don't add 3G interface), 2012/03/29
#if (defined(CTCONFIG_3G_FEATURE) && !defined(CONFIG_BCM_ADSL_MODULE))
                if( from_dev->name==NULL || (strstr(from_dev->name,"ppp") && strlen(from_dev->name)<5))
                {
                   if(from_dev)
                    dev_put(from_dev);
                   continue;
...
//<< [CTFN-3G-001-4] evan : Support 3G feature without xDSL driver (the function br_igmp_mc_fdb_add don't add 3G interface), 2012/03/29
#if (defined(CTCONFIG_3G_FEATURE) && !defined(CONFIG_BCM_ADSL_MODULE))
          if( dev->name==NULL || (strstr(dev->name,"ppp") && strlen(dev->name)<5))
            ;
          else
#endif
//>> [CTFN-3G-001-4] End
