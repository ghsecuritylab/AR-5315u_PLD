...
//<< [CTFN-NMIS-008] Charles Wei, Support the unspecified source port for IPSec pass-through 2011/04/18
         if ( udph->source == IPSEC_PORT && udph->dest == IPSEC_PORT ) {
...
//<< [CTFN-NMIS-008] Charles Wei, Support the unspecified source port for IPSec pass-through 2011/04/18
        if ( ipsec_entry->port_masq_flag == 1 )
                ipsec_outbound_pkt(&skb, ct, ctinfo);
	ret = NF_ACCEPT;
...
//<< [CTFN-NMIS-008] Charles Wei, Support the unspecified source port for IPSec pass-through 2011/04/18
static struct nf_conntrack_helper ipsec;
static struct nf_conntrack_expect_policy ipsec_exp_policy;

static int __init nf_conntrack_ipsec_init(void)
{
   ipsec_exp_policy.max_expected = 1;
   ipsec_exp_policy.timeout	= 5 * 60;
   ipsec.name = "ipsec";
   ipsec.me = THIS_MODULE;
   ipsec.tuple.src.l3num = AF_INET;
   ipsec.tuple.dst.protonum = IPPROTO_UDP;
   ipsec.tuple.src.u.udp.port = htons(IPSEC_PORT);
   ipsec.expect_policy = &ipsec_exp_policy;
   ipsec.expect_class_max = 1;
   ipsec.help = conntrack_ipsec_help;
//>> [CTFN-NMIS-008] End
