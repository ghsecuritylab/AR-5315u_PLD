...
/******************************************************************************
 *  we modify the driver of hso, and support a new device of alcatel datacard.
 *  write by: Nik Du
 *  date    : 2009/1/9
 *****************************************************************************/
...
/****************************************************************
 *   if we want support our datacard  device, we must modify the
 *   VID and PID of USB for matching our alcatel data device.
 *   write by: Nikdu
 *   data: 2009/1/8
 ****************************************************************/
...
static const struct usb_device_id hso_ids[] = {
	{default_port_device(0x1bbb, 0x0017)},
	{default_port_device(0x1bbb, 0x0017)},
	{default_port_device(0x1bbb, 0x0017)},
	{default_port_device(0x1bbb, 0x0017)},
	{default_port_device(0x1bbb, 0x0017)},
	...
	{default_port_device(0x1bbb, 0x0017)},
	{default_port_device(0x1bbb, 0x0017)},
	{default_port_device(0x1bbb, 0x0017)},	/* Icon-Edge */
	{icon321_port_device(0x1bbb, 0x0017)},	/* Module HSxPA */
	{icon321_port_device(0x1bbb, 0x0017)},	/* Icon-321 */
	{icon321_port_device(0x1bbb, 0x0017)},	/* Icon-322 */
	{USB_DEVICE(0x1bbb, 0x0017)},		/* GE40x */
	{USB_DEVICE(0x1bbb, 0x0017)},		/* GE40x */
	{USB_DEVICE(0x1bbb, 0x0017)},		/* GI 0401 */
	{USB_DEVICE(0x1bbb, 0x0017)},		/* GTM 382 */
	{USB_DEVICE(0x1bbb, 0x0017)},		/* GE40x */
	{}
};
...
static int hso_get_config_data(struct usb_interface *interface)
{
	//struct usb_device *usbdev = interface_to_usbdev(interface);
        /* if we support alcatel data device, we need set some default value.*/  
    	
	static u8 config_data[17]= {0x01,0x04,0x04,0x08};	
	u32 if_num = interface->altsetting->desc.bInterfaceNumber;
	s32 result;
	
        /******************************************************
         *  if we support alcatel datacard, we don't need use 
         *  it get config data.
         ******************************************************/
#if 0	
		     ...
		     const struct usb_device_id *id)
{
	...
	/* if we want support alcatel data card, we need del some data.*/
#if 0        
 	if (id->driver_info)
	...
	/* for support alcatel data device,we modify the interfaceClass to match.*/

	if (interface->cur_altsetting->desc.bInterfaceClass != 255) {
...
//<< add by dylan
	printk("%s %d: GSM modem (1-port) converter detected\n" , KBUILD_MODNAME, dev2ser(hso_dev)->minor);
	printk("%s: GSM modem (1-port) converter now attached to /dev/%s%d\n" , KBUILD_MODNAME, tty_filename, dev2ser(hso_dev)->minor);
//>> add by dylan	
...
//<<add by dylan
	printk("%s: GSM modem (1-port) converter now disconnected\n" , KBUILD_MODNAME);
//>>add by dylan	
}
...
//<<add by dylan
	printk(KERN_INFO KBUILD_MODNAME ": " DRIVER_VERSION ":"  MOD_DESCRIPTION "\n");
//>>add by dylan
	/* done */
	return 0;
}
