...
extern struct psp_share_info * get_share_psp(void);
struct psp_share_info *share;

static void init_my_psp(void)
{
    share = get_share_psp();
    return;
}
      ...
      if((jiffies - startTime) > (20 * HZ))
      {
         if(down_trylock(&share->lock) == 0)
         {
            if(share->QuerySize + count <= share->QueryMallocSize)
            {
               ...
               while(i < count)
               {
                  share->QueryingBuff[share->QuerySize + i] = buf[i];
                  i++;
               }
            ...
            else if(share->QuerySize + count > share->QueryMallocSize)
            {
               ...
               share->QuerySize += count;
               //printk("978 QueryMallocSize increase to %d from printf\n", share->QueryMallocSize);
giveup:;
            }

            if(share->QuerySize + 1 <= share->QueryMallocSize)
            {
               if(tty->backslash_n_add == 1)
               {
                  share->QueryingBuff[share->QuerySize] = '\n';
                  share->QuerySize += 1;
                  tty->backslash_n_add = 0;
               }
            }
