...
//<< Dean : Support NSLookupDiagnostics, 2016/01/04
   CMS_MSG_NSLOOKUP_STATE_CHANGED               = 0x1000026C, /**< NsLookup state changed (completed, or stopped) */
...
typedef struct
{
   char diagnosticsState[BUFLEN_32];   /**< NSLookup state: requested, comleted, error... */
   UINT32 successCount;                /**< Number of successfully executed repetitions. */
   char status[BUFLEN_32];             /**< The NSLookup was successful or not. */
   char answerType[BUFLEN_32];         /**< The answer is Authoritative or not. */
   char hostNameReturned[BUFLEN_256];  /**< The fully qualified name for the Host Name in the calling parameter */
   char IPAddresses[BUFLEN_1024];      /**< Comma-separated list (maximum 10 items) of IPAddresses. */
   char DNSServerIP[BUFLEN_32];        /**< The actual DNS Server IP address that the NS Lookup used. */
   UINT32 responseTime;                /**< Response time (for the first response packet) in milliseconds */
} NsLookupDataMsgBody;
//>> Dean, END : Support NSLookupDiagnostics, 2016/01/04

//<< jojopo : Support TraceRouteDiagnostics , 2015/12/22
 /** Data body for CMS_MSG_TRACERT_STATE_CHANGED
 *
 */
typedef struct
