...
//<< [BCMBG-SYS-007] Fix CPE cannot upgrade whole flash image (.w), 2012/04/12                     
//   return (flashSize > (imageSize + CMS_IMAGE_OVERHEAD));
   return ((flashSize+ CMS_IMAGE_OVERHEAD) > imageSize );
//>> [BCMBG-SYS-007] End
}
...
void cmsUtil_sendKillCT_watchdog(void *msgHandle){
	CmsMsgHeader msg = EMPTY_MSG_HEADER;
	msg.type = CMS_MSG_STOP_APP;
	msg.src = cmsMsg_getHandleEid(msgHandle);
	msg.dst = EID_SMD;
	msg.flags_request = 1;
	msg.wordData = EID_CT_WATCHDOG; 
	cmsMsg_sendAndGetReply(msgHandle, &msg);
}

void cmsImg_stopWatchDog(void *msgHandle){
	  cmsUtil_sendKillCT_watchdog(msgHandle);
