...
UINT32 devCtl_getWirelessBand1(void)
{
    CmsRet ret = CMSRET_SUCCESS;
    int band1 = 0;
    ret = devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ, GET_WLBAND1, "", 0, 0, &band1);
    return band1;
}
UINT32 devCtl_getWirelessBand2(void)
{
    CmsRet ret = CMSRET_SUCCESS;
    int band2 = 0;
    ret = devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ, GET_WLBAND2, "", 0, 0, &band2);
    return band2;
}
//>> [CTFN-WIFI-014] End
...
UINT32 devCtl_getWirelessButton(void)
{
    CmsRet ret = CMSRET_SUCCESS;
    int flag = 0;
    ret = devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ, GET_WIFI_BUTTON, "", 0, 0, &flag);
    return flag;
}
...
CmsRet devCtl_setEOCOption(int option)
{
   ...
   nvramData.ctData.EOC_Option = (UINT32)option;
...
// >> [CTFN-SYS-024-1] end
//<< [CTFN-WIFI-014] Support two Wireless Interfaces with band defined in board parameter and modify SSID according to band info
                  (boardIoctl == BOARD_IOCTL_FLASH_READ && action == GET_WLBAND1) ||
                  (boardIoctl == BOARD_IOCTL_FLASH_READ && action == GET_WLBAND2) ||
//>> [CTFN-WIFI-014] End
//<< [CTFN-SYS-039] jojopo : Support to save console log on scratch pad , 2013/11/01
#ifdef CTCONFIG_SYS_LOG_ON_SCRATCH
                  (boardIoctl == BOARD_IOCTL_FLASH_READ && action == SCRATCH_PAD_SIZE) ||
                  (boardIoctl == BOARD_IOCTL_FLASH_READ && action == SCRATCH_PAD_AVAILABLE_SIZE) ||
                  (boardIoctl == BOARD_IOCTL_FLASH_READ && action == SCRATCH_PAD_STARTSECTOR) ||
                  (boardIoctl == BOARD_IOCTL_FLASH_READ && action == SCRATCH_PAD_RAWDATA) ||
                  (boardIoctl == BOARD_IOCTL_FLASH_READ && action == SCRATCH_PAD_BOOTCOUNT) ||
                  (boardIoctl == BOARD_IOCTL_FLASH_READ && action == SCRATCH_PAD_BOOTDATAANDSIZE) ||
                  (boardIoctl == BOARD_IOCTL_FLASH_READ && action == SCRATCH_PAD_CLEARALLPAD) ||
                  (boardIoctl == BOARD_IOCTL_FLASH_READ && action == SCRATCH_PAD_CALLTRACE) ||
                  (boardIoctl == BOARD_IOCTL_FLASH_READ && action == SCRATCH_PAD_ADDDEL_KEYWORD) ||
                  (boardIoctl == BOARD_IOCTL_FLASH_READ && action == SCRATCH_PAD_GET_KEYWORD_LIST) ||
#endif
//>> [CTFN-SYS-039] end
//<< [CTFN-WIFI-001] kewei lai : Support two Wireless en/disable buttons and Wireless/WPS in one button
				(boardIoctl == BOARD_IOCTL_FLASH_READ && action == GET_WIFI_BUTTON) ||
//>> [CTFN-WIFI-001] End
