...
//>> [CTFN-NWAN-005] End
	...
	{
//<< [BCMBG-NTWK-083] Charles Wei: Fix CPE cannot establish PPPoE session in the environment which has multiple BRAS at the same time if xDSL retrain
//<< WenXing : fix bug muti-pado issue, 2009.1.19		
		if(ses->state == PADS_CODE) {
		    //poe_dbglog(ses, "#### SES retransmits =%d [%d]###\n",ses->retransmits, __LINE__);
		    --ses->retransmits;
		    //poe_dbglog(ses, "#### SES retransmits =%d [%d]###\n",ses->retransmits, __LINE__ );
		}
		else{
//>> WenXing : end, fix bug muti-pado issue, 2009.1.19
//>> [BCMBG-NTWK-083] End
	    if(ses->rcv_pado){
