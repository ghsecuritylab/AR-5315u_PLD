...
extern void sendEventMessageOption121(int Len , unsigned char *option);
unsigned char option121[128] = {0};
int opt121Len = 0;
...
//<< [CTFN-NRUT-002] Leo Ting :  Support DHCP option 121 for DHCP client
#ifdef CTCONFIG_NTWK_DHCPC_OPT121
			if (!strcmp(type_p->name, "clsrouter"))
			{
			   cflag = 1;
            printf("Receive dhcp option121 data.\n");

			for(i=0 ; i<option[OPT_LEN - 2]+OPT_LEN ; i++)
            {
               option121[i] = option[i-1] ;
            }
            //for(i=0 ; i<option[OPT_LEN - 2]+OPT_LEN ; i++) printf("%02x\n" , option121[i]); //for debug

            opt121Len = option[OPT_LEN - 2]+OPT_LEN;
			}
#endif /* CTCONFIG_NTWK_DHCPC_OPT121 */
//>> [CTFN-NRUT-002] Leo Ting :  Support DHCP option 121 for DHCP client		
 			break;
...
//<< [CTFN-NRUT-002] Leo Ting :  Support DHCP option 121 for DHCP client
#ifdef CTCONFIG_NTWK_DHCPC_OPT121
	  if(opt121Len > 0) sendEventMessageOption121(opt121Len , option121);
#endif /* CTCONFIG_NTWK_DHCPC_OPT121 */
//>> [CTFN-NRUT-002] Leo Ting :  Support DHCP option 121 for DHCP client
