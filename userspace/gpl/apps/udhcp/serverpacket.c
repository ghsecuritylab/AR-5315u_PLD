...
int isHostFromWireless(u_int8_t *chaddr)
{
   char cmd[128] = {0};
   char line[256];
   ...
   char interface[16];
   FILE *fp;
   int portNo, wifiMask = 0, isWifi = 0;
   int i;
   unsigned char *p = macAddr;
   //unsigned char *q = chaddr;

   sprintf(cmd, "brctl showmacs br0 > /var/tmpshowmacs");
   system(cmd);

   sprintf(cmd, "brctl showstp br0 > /var/tmpshowstp");
   system(cmd);


   fp = fopen("/var/tmpshowstp", "r");
   if(fp) {

      while ( fgets(line, sizeof(line), fp) ) {
         portNo = 0;
         sscanf(line, "%s (%d)", interface, &portNo);

         if(portNo == 0)
            continue;

         if(strstr(interface, "wl")) {
            wifiMask |= 1 << portNo;
         }
      }
   ...
   fp = fopen("/var/tmpshowmacs", "r");
   if(fp) {

      while ( fgets(line, sizeof(line), fp) ) {

         sscanf(line, "%d %02x:%02x:%02x:%02x:%02x:%02x",
            &portNo, &addr[0], &addr[1], &addr[2], &addr[3], &addr[4], &addr[5]);
         ...
         if(memcmp(p, chaddr, 6) == 0) {

            if(wifiMask & 1 << portNo)
               isWifi = 1;
