...
//<< [BCMBG-NTWK-109] Lucien Huang : Enlarge the DNS proxy handling buffer and ignore DNS answer larger than the buffer size, 2014/01/15
  /* check that the message is too long, the extra packet length in dns_construct_reply is 16 bytes */
  if( m->numread > (MAX_PACKET_SIZE - 16))
  {
    debug("dns_read_packet: packet too long to be dns packet");
    return -1;
  }
//>> [BCMBG-NTWK-109] End
