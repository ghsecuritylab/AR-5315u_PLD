...
//<< [BCMBG-NTWK-086] jojopo : Fix static DNS cannot work when DNS proxy is enabled in profile, 2012/08/30
      /* fix incorrect memfree */
      DNSInfoEntry *tmp = curr;
      ...
      //printf("[[[[Free subnet=%s, mask=%s, ipv4_dns1=%s, ipv4_dns2=%s, ipv6_dns1=%s, ipv6_dns2=%s ]]]]\n",
      //              inet_ntoa(curr->lanSubnet), inet_ntoa(curr->lanMask), curr->ipv4_dns1, curr->ipv4_dns2, curr->ipv6_dns1, curr->ipv6_dns2);
