...
#define bcmPktDma_tmPortConfig(_port, _mode, _kbps, _mbs)               \
    ({                                                                  \
        int __ret = 0;                                                  \
        if(bcmPktDma_hostHooks_g.tmPortConfig != NULL)                  \
            __ret = bcmPktDma_hostHooks_g.tmPortConfig(_port, _mode, _kbps, _mbs); \
        __ret;                                                          \
    })
...
typedef struct {
    ...
    int (*tmPortConfig)(uint8 port, fapTm_mode_t mode, int kbps, int mbs);
    int (*tmPortMode)(uint8 port, fapTm_mode_t mode);
    int (*tmPortApply)(uint8 port, int enable);
#endif
