...
static void devname_by_phyid(char* name, int phyid){
	...
	char buf[IFNAMSIZ];
	int i;
//<< [TTS3619] David Rodriguez: Reorder AR-5315u LAN ports (LAN1-eth0, LAN2-eth1, LAN3-eth2...), 2013-08-02
	char tempboardid[32];
	int n = 0;
//>> [TTS3619] End
...
//<< [TTS3619] David Rodriguez: Reorder AR-5315u LAN ports (LAN1-eth0, LAN2-eth1, LAN3-eth2...), 2013-08-02
			...
			if(vport_cnt == 4)
			{
				BpGetBoardId(tempboardid);
				if(strstr(tempboardid, "96318A-1441N1")) /* AR-5315u */
				{
					n = ((4-phyid) >= 0) ? (4-phyid) : (phyid-1);
					snprintf(buf, IFNAMSIZ, name, n);
				}
				else
				{ // default
					snprintf(buf, IFNAMSIZ, name, (phyid-1));
				}
			}
#endif
//>> [TTS3619] End
