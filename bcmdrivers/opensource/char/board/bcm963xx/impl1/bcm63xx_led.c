...
//<< [CTFN-WIFI-017]kewei lai: Support the second WPS LED
		{kLedSes2, BpGetCtLedSesWireless2, NULL},
// << [CTFN-WIFI-017] End
//>> [CTFN-SYS-032-6] kewei lai : Support the second USB LED
	{kLedUsb1Single, BpGetCtUsbSingleLed1Gpio, NULL},
	{kLedUsb2Single, BpGetCtUsbSingleLed2Gpio, NULL},
//<< [CTFN-SYS-032-6] End
...
//<<for NL-3110u
    ...
    /* Enable LED controller to drive this GPIO */
    if (!(led_gpio & BP_GPIO_SERIAL) && (led_gpio & 0xFFFF) != BP_GPIO_9_AL)
        GPIO->LEDCtrl |= GPIO_NUM_TO_MASK(led_gpio);
#endif

#if defined(CONFIG_BCM963268)
	if((led_gpio & 0xFFFF) != BP_GPIO_9_AL){
...
#elif defined(CONFIG_BCM96328) || defined(CONFIG_BCM96362) ||defined(CONFIG_BCM96318)
    LED->ledMode &= ~(LED_MODE_MASK << GPIO_NUM_TO_LED_MODE_SHIFT(led_gpio));
    if( gpio_state )
        LED->ledMode |= (LED_MODE_OFF << GPIO_NUM_TO_LED_MODE_SHIFT(led_gpio));
    else
