...
#define FLASH_SST       3
//<< [CTFN-SYS-004] Jimmy Wu : Add new flash for Comtrend
...
#define FLASH_PROG_BLOCK	9
#define FLASH_MRESET		10
//>> [CTFN-SYS-004] End
...
#define ID_W19B320BB    0x222A

#define ID_MX29GL128EH	0x2221
#define ID_MX29GL256EH	0x2222
//>> [CTFN-SYS-004] End
...
#define CFI_FLASH_DEVICES                   \
         {{ID_AM29DL800T, "AM29DL800T"},    \
          ...
          {ID_W19B320BB, "W19B320BB"},      \
          {ID_MX29GL128EH, "MX29GL128EH"},  \
          {ID_MX29GL256EH, "MX29GL256EH"},  \
          {0, ""}                           \
        }
//>> [CTFN-SYS-004] End
...
//<< [CTFN-SYS-004] Jimmy Wu : Add new flash for Comtrend
    int block_pagesize, wordsize;
    int temp_offset;
    unsigned char *temp_buffer,*word_p;
//<< Antony : do not activate cfi_flash_write_to_buffer
    if( 0 && meminfo.write_buffer_size != 0 && offset == 0 &&
...
//<< [CTFN-SYS-004] Jimmy Wu : Add new flash for Comtrend
    ...
    else
    {
        ...
        //printk(" sector=%x, numbytes=%x, block_pagesize=%x, wordsize=%d, offset=%x\n", sector, numbytes, block_pagesize, wordsize, offset);
        for( i = 0; i < 3; i++ ) {
            //Block Programming
            if (block_pagesize > 0)
...
//<< [CTFN-SYS-004] Jimmy Wu : Add new flash for Comtrend
        case FLASH_BLOCK:
            ...
            flashptr[0x2AA] = 0x55;
            flashptr[0] = 0x25;
            flashptr[0] = 0x0F;		/*Write buffer 16 bytes for each time*/
            break;
...
//<< asdrick : Add protection to avoid read/write error for mirror bit Flash
#ifndef _CFE_
	down(&cfi_flash_lock);
#endif
//>> asdrick : End
	...
	{
		cfi_flash_command(FLASH_BLOCK, sector, offset, *src, NULL);
		...
		cfi_flash_command(FLASH_PROG_BLOCK, sector, 0, 0, NULL);
		offset-=2;
		src--;
		...
		cfi_flash_command(FLASH_MRESET, 0, 0, 0, NULL);
